...
(`combine_results`, pfr_id-linked), landing spot (the drafting team's *prior* season pass
...
and college production (`college_usage`, espn_id-linked, the player's *final college season
only* — same prior-season-only rule as landing spot, never the rookie's own NFL season).

College production was absent through D20 — no verified ID bridge to cfbfastR-data existed
at the time. D38 found and verified a real bridge via `espn_id` (see features/college_production.py),
using CFBD directly rather than cfbfastR-data's mirror.
