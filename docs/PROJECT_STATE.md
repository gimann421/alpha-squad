...
## Status: M25 complete — Candidate C demand-target refinement (D66); **no production change**. M13 complete; M14 post-audit hardening (D41-D52) except P2-3 (needs a user credential decision); M15 productization (D53); M16-M19 evaluation, draft forensics, opportunity cost, 1-QB retarget (D54-D60); M20 benchmark artifact found (D61); M21 measurement fixed (D62); M22 shipped `msv + VORP` (D63); M23 rejected two kicker fixes (D64); M24 proved static replacement is a causal defect (D65); M25 found demand DEPTH is the governing parameter and identified variants passing every gate (D66). **P1-0 remains OPEN**: production N4 is 5.6 starter points behind fair consensus. Diagnostic scale x2.5 reaches +21.1 vs consensus, passes all gates, and generalizes out-of-format (+65.6 on the 2QB config) -- pending a pre-registered confirmation run before it can ship.
