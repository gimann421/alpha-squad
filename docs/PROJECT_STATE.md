...
| M14 Post-audit hardening | IN PROGRESS | Working the `docs/CURRENT_STATE_AUDIT.md`/`docs/IMPLEMENTATION_GAP_ANALYSIS.md` P0-P3 backlog. P0 (security): D35's history-rewrite decision was already made and declined by the user at the time -- not reopened; added a durable CI guardrail (`scripts/check_no_secrets.py`, D42) against a repeat instead. P1-2 (EDGE backtest): `alpha-squad edge backtest` + `reports/edge_backtest.md` (D41), real per-position/bucket breakdown, BUY beat market in all 4 scored seasons 2022-2025. P1-3 (model persistence): `models/persistence.py` (D43) -- the audit's single biggest architectural finding (no model was ever saved to disk) is now closed for the two paths that actually serve live predictions (uncertainty → `/rankings`, rookie projection → `/rookies`); verified against the real database, not just fixtures. 273 offline tests passing (up from 262 at audit time). Remaining backlog (P1-1 UI wiring, P2-P3) in progress. |
