...
| Does league-aware decision-making improve on generic/BPA rankings? | **VALIDATED, decisively** — under the 1-QB target format's corrected benchmark, `alpha_league_aware` beats `alpha_bpa` on both starter points (1990.9 vs 463.5) and total roster points; `alpha_bpa`'s collapse is real (verified against drafted rosters: 15 QBs/1 K, zero RB/WR/TE/DST), a clean demonstration that roster-aware value is load-bearing, not cosmetic, once the benchmark is measuring the right format (D59/D60; the pre-D56 "MIXED" verdict below described the superflex board) | `evaluation/draft_simulation.py`, `tests/unit/test_evaluation_draft_simulation.py` | `reports/draft_simulation.md` |
| Do draft recommendations outperform market consensus? | **MET.** Root cause was two-fold: two real engine defects (D54's saturation penalty, D55's missing positional opportunity cost, both fixed) plus a benchmark measuring the wrong market — every pre-D56 result above was scored against `ecr_type='rsf'`, FantasyPros' superflex board, appropriate for the pre-D58 2QB target and not the current 1-QB one (D56). Re-baselined on the corrected `ro` board with no engine change, `alpha_league_aware` already led consensus (+102.6, D59). A pre-registered ablation then found and fixed a further real defect — VORP pricing bench K/DST at positive value despite zero flex eligibility — via marginal starter value (D60). **Official benchmark, full 2021-2025, determinism-verified: 1990.9 vs. 1825.2 mean starter points (+165.7, +9.1%), 37/50 (74%) win rate, wins 4 of 5 seasons.** See `docs/IMPLEMENTATION_GAP_ANALYSIS.md` P1-0 (closed) and `docs/FORMAT_MIGRATION_DIAGNOSTIC.md` for the full account | same as above | same as above |
