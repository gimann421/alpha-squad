...
| Draft/waiver/FAAB recommendations can be generated | ✅ | D44: draft form in `LeagueView.tsx` (pre-existing) plus new `WaiverView.tsx`/`TradeView.tsx` — all three live-tested against `dilworth` (real Sleeper league, season 2025); prior to D44 this row's citation ("league draft/waiver forms in `LeagueView.tsx`") was inaccurate — only the draft form existed, per `docs/CURRENT_STATE_AUDIT.md`'s UI/API sub-audit |
