...
> **Last reconciled with `docs/DECISIONS.md`: D71.** The draft-benchmark rows under *Empirical
> validation* below were frozen at D60 — a result `docs/DECISIONS.md` D61 retracted — and are
> corrected here. `docs/PROJECT_STATE.md`'s status header remains the authority on current
> milestone state.
...
| Target league settings are supported | ✅ | `config/league_configs/target_league.yaml` — 10 teams, `QB1/RB2/WR2/TE1/FLEX2/K1/DEF1` = 10 starters, bench 6, `roster_size` 16 (D58, replacing the pre-D58 2QB/2RB/2WR/1TE/2FLEX target). `legacy_2qb_dynasty.yaml` keeps the prior format registered so a second, genuinely different format stays exercised |
| Multiple leagues can be configured and switched between | ✅ | `league/context.py::resolve_league` + `config/league_configs/registry.yaml` (D33/D34) — every CLI league command, the API, and `LeagueView.tsx`'s league picker resolve through the same registry; 4 leagues registered today (2 YAML — target and legacy 2QB, 2 live Sleeper), no hardcoded single-league or single-format assumption remains |
| Replacement level is calculated | ✅ | `league/replacement.py::replacement_level`, derived from the league's own starting-slot config (verified: a 2QB league's QB replacement sits at real rank ~20-24 vs. the 1-QB target's ~12-15) |
| Positional capacity/scarcity are derived from the league config, never hardcoded per position | ✅ | `league/roster.py::positional_capacity`/`startable_slots` (D58) — every slot a position could start in (dedicated + eligible flex) plus a bench share proportional to that, verified flex-aware: target league QB 2, RB 6, WR 6, TE 4, K 2, DST 2. `positional_scarcity` (`league/replacement.py`) is consulted by `league/waiver.py` but deliberately **not** by the draft engine: M17/M18 measured it raising the RB=0 rate 20%→32% under the pre-D56 superflex board; the draft engine instead prices forward-looking positional value via `league/opportunity_cost.py` (D55) and marginal starter value (D60) |
| Roster fit is calculated | ✅ | `roster_fit_multiplier`, depth target now derived from `startable_slots` rather than the pre-D58 constant `slots + 2` |
| Expected pick value exists | ✅ | `recommend_draft_pick`: `(marginal_starter_value + positional opportunity cost) × roster fit × confidence × survival × [feasibility cap]` when the caller supplies the roster's real player ids; falls back to `(VORP + opportunity cost) × ...` (D55) otherwise — see D60 |
| The consensus/market benchmark matches the target league's format | ✅ | `market/series.py::resolve_market_series` (D56) — derives `(ecr_type, page_type)` from the league's own lineup/format (superflex detection, dynasty vs. redraft), rather than a fixed default. Verified against real data that the pre-D56 fixed default (`rsf`) is FantasyPros' superflex board (9 of the real preseason-2024 overall top 15 are QBs) and the 1-QB target's board (`ro`) is not (0 of 15) |
| K and DEF starting slots are supported | ✅ | `features/kicking_defense.py` (real computed scoring + `asq_dst_{team}` entities, D57), `models/baselines/kicking_defense.py` (measured baselines, not models — K r=0.41/DST r=0.29 year-over-year on real 2015-2025 data ruled out an ML model) |
...
| Does league-aware decision-making improve on generic/BPA rankings? | **VALIDATED, decisively** — under the 1-QB target format's corrected benchmark, `alpha_league_aware` beats `alpha_bpa` on both starter points (1990.9 vs 463.5, D60-era figures) and total roster points; `alpha_bpa`'s collapse is real (verified against drafted rosters: 15 QBs/1 K, zero RB/WR/TE/DST), a clean demonstration that roster-aware value is load-bearing, not cosmetic, once the benchmark is measuring the right format (D59/D60; the pre-D56 "MIXED" verdict below described the superflex board). **The verdict is unchanged under the current benchmark** even though the D60 figures are superseded: D71 measures **0/50 unfilled mandatory starting slots for `alpha_league_aware` against 327/50 for `alpha_bpa`** | `evaluation/draft_simulation.py`, `tests/unit/test_evaluation_draft_simulation.py` | `reports/draft_simulation.md` |
| Do draft recommendations outperform market consensus? | **INCONCLUSIVE — not resolvable at the current data scale (D71).** This row previously recorded the D60 closure (**1990.9 vs. 1825.2, +165.7/+9.1%, 37/50**) as **MET**; **D61 retracted that result** — the benchmark's `market_consensus` opponent left two of its ten starting slots empty, an artifact worth **+245.3** starter points, more than the whole margin. Re-measured against the fair `market_consensus_roster_aware` opponent, Alpha was **−45.4** (D62). Engine work then closed most of that gap on pre-registered ablations — the `msv + VORP` value base (D63) and draft-aware replacement at the demand a draft actually consumes (D67) — and D67's official benchmark has Alpha **2061.3 vs. 2034.8 (+26.5), W/L 28/22**. **The lead is not statistically established:** the season-clustered 95% CI is **[−100.4, +153.4]** (the 50 trials are a deterministic season × slot cross-product measuring n=5 seasons, not 50 independent draws — D71), and the margin falls to **+3.1** with 2022 removed. **D68–D70 changed no production behavior** — every positional-calibration arm was rejected (D68), RB-only calibration was abandoned before implementation (D69), and RB availability features failed their pre-registered gates (D70). **D71 re-specified the exit criterion and split the item: P1-0a (roster-balance defect) CLOSED; P1-0b (absolute superiority vs. fair consensus) OPEN**, reclassified as a standing measurement limitation rather than a blocking gate — resolving a +26.5 effect needs ~61–116 seasons against a universe hard-bounded at 2021+ and growing one per year. Further RB/WR/QB experimentation is **not currently justified**; future engine changes are evaluated by variant-vs-control contrast on identical trials instead. The immediate *implementation* issue is the production/benchmark configuration gap: three of four production entry points omitted `roster_player_ids` and so ran the un-benchmarked VORP-only value base rather than the D63/D67 roster-aware one. See `docs/IMPLEMENTATION_GAP_ANALYSIS.md` P1-0a/P1-0b and `docs/DRAFT_STRATEGY_FORENSIC_ANALYSIS.md` | same as above | same as above |
