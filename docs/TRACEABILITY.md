...
| FantasyPros API integration works or limitation documented | ⚠️ | `sources/fantasypros.py` implemented; network layer confirmed open 2026-08-22 (D31). `FANTASYPROS_API_KEY` supplied 2026-08-23 but rejected by FantasyPros's own API (`403 Forbidden`, D36) — not a policy block or missed env var, root cause not established, needs the user to check the key at FantasyPros's dashboard; substituted by DynastyProcess `db_fpecr` (D3) for the ECR signal itself in the meantime |
