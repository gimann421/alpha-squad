...
the waiver/trade/roster-need decision engines too — **D43 (`docs/DECISIONS.md`) closed that gap**,
...
league" is smaller than it was at session start, and is now smaller still after D43: usage evidence
...
| Waiver/FAAB decisions | COMPLETE/VERIFIED | `waiver.py`, live-tested; D43 wired it into `WaiverView.tsx` and recorded real `waiver_bid` decisions from the UI (previously zero) |
| Roster-aware decisions | COMPLETE/VERIFIED | Roster-fit math is real; D43 added a UI surface (`LeagueView.tsx`'s "Roster need" section) that calls it (§20) |
...
| Item | Status | Evidence |
|---|---|---|
| Interface exposes the validated player intelligence | COMPLETE/VERIFIED | 8-tab SPA (D43 added Waiver/Trade), thin projection over API, no logic duplication (UI/API sub-audit) |
...
| Roster-aware recommendations can be generated | COMPLETE/VERIFIED | D43: `LeagueView.tsx`'s "Roster need" section calls `GET /league/{id}/roster` and renders the real `need` map; live-tested against `dilworth` (real Sleeper league) |
| Draft/waiver/FAAB recommendations can be generated | COMPLETE/VERIFIED | D43: `WaiverView.tsx` (new) and `TradeView.tsx` (new) now wire `postWaiver`/`postTrade` into reachable tabs, alongside the pre-existing draft form; all three live-tested against `dilworth` (real Sleeper league, season 2025) with real recommendations and reasons rendered |
| Recommendation explanations show relevant evidence/provenance | IMPLEMENTED/UNVERIFIED | D43 newly confirms `reasons` renders in all three league-decision views (draft, waiver, trade); still not independently re-confirmed for every other view (Rankings/EDGE/Evidence/Rookies) this session, so the broader status is left as-is rather than overclaimed |
...
| `web/` | React/Vite/TS SPA, 8 tabs | Real; D43 wired waiver/trade/roster-need — simulation remains server-only, no API/UI exposure |
...
now that the UI actually calls all three (D43), real `waiver_bid` and `dynasty_trade` decisions were
recorded for the first time this session (against a working copy of the database, not this
deployment's original `data/alpha_squad.duckdb`, since D43 ran in an isolated worktree with no prior
ingest of its own).
...
**D43 closed the waiver/trade/roster-need gap** this section previously flagged: `WaiverView.tsx` and
...
real `dilworth` Sleeper league (season 2025) — see D43 in `docs/DECISIONS.md` for the exact
...
3. **Simulation is fully built and tested but invisible in the application** (§20) — real engineering
   effort that delivers zero end-user value today because no API endpoint or UI routes to it. (D43
...
- **P1:** ~~Wire waiver/trade/roster-need into the UI~~ **done (D43)**; commit an EDGE
...
- Load a real registered Sleeper league (`dilworth` or `boys_of_fall`) and get a real, league-specific
  ...
  session), and — as of D43 — a real waiver FAAB-bid recommendation, a real dynasty trade evaluation,
  and a real roster-need breakdown, all from UI buttons on the Waiver/Trade/League tabs (verified live
  against `dilworth` this session: see `docs/DECISIONS.md` D43 for the exact numbers observed).
...
unresolved. (As of D43, this is an improvement over the prior draft of this audit: waiver, trade, and
...
in §5's table above (higher now that D43 moved the Application/interface rows in §5 off PARTIAL);
...
roster-need held this position in the prior draft of this audit; D43 closed that gap.)
...
web UI~~ — **done, D43.** The next-highest-value, lowest-risk increment in the same spirit is wiring
